...
import * as Checbok from '@radix-ui/react-checkbox'
              ...
              <Dialog.Title className="text-3xl font-black">Publique um anúncio</Dialog.Title>
                <form onSubmit={handleCreatedAd} className='mt-8 flex flex-col gap-4'>
                  <div className='flex flex-col gap-2'>
                    ...
                    <select className='bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 outline-0' name="game" id="game" defaultValue="">
                      <option selected disabled className="text-zinc-500" value="">Selecione o game que deseja jogar</option>
                       {
                        games.map(game => {
                            return  <option key={game.id} className="text-white" value={game.id}>{game.title}</option> 
                        })
                       }
                    </select>
                  </div>
                  ...
                  <div className="mt-2 flex items-center gap-2 text-sm">
                    <Checbok.Root checked={useVoiceChannel}  onCheckedChange={(checked) => {
                        if(checked == true){
                  ...
                  <footer className='mt-4 flex justify-end gap-4'>
                    ...
                    <button type='submit' className='bg-violet-500 flex items-center gap-3 px-5 h-12 rounded-md font-semibold hover:bg-violet-600'><GameController className='w-6 h-6'/> Encontrar duo</button>
                  </footer>
                </form>
              </Dialog.Content>
          </Dialog.Overlay>
